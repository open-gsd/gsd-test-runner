...
Future direction: the macOS Bench today runs Docker-on-macOS (Docker Desktop or colima, running the Linux Tester Image — see ADR-0020, amended 2026-05-24), which gives filesystem isolation but not macOS-native sandbox parity, since the guest is still Linux. The current macOS Runner is otherwise bare-metal `node --test` with no memory ceiling. ADR-0030 (2026-09-06) rejects Apple Containers permanently — it is architecturally Linux-guest-only, independent of macOS version — and instead proposes **Tart** (Apple's Virtualization.framework, wrapped by a CLI) as a fourth Runner target: a genuine macOS guest with a hypervisor-enforced memory cap, closing both the "test real macOS code paths" gap and the bare-metal Runner's out-of-memory failure mode. The `Runtime` field and `RuntimeContainer` constant remain in the codebase as a record that Apple Containers was evaluated and rejected; `RuntimeTart` is the new, not-yet-implemented path (see ADR-0030 Decisions 3–4 for open implementation questions).
...
### macOS Containers (Apple Containers) — evaluated and rejected

Apple's native container runtime for macOS 26+ (CLI: `container`, not `docker`). Despite early framing to the contrary, Apple Containers runs **Linux** containers only — every container is its own lightweight Linux VM (confirmed against Apple's own `container`/`containerization` docs); there is no macOS-guest mode and no announced roadmap for one. It therefore cannot give macOS-native sandbox parity, independent of macOS version availability. `bench.RuntimeContainer` and the `Runtime` selector on `bench.Bench` remain in the codebase (`internal/dockerexec` would invoke the `container` binary when `Bench.Runtime == RuntimeContainer`) purely as a record that this path was evaluated twice — ADR-0020 (2026-05) and ADR-0030 (2026-09) — and rejected both times on its technical merits, not availability. `dockerfiles/macos.containerfile` was deleted as part of ADR-0020's 2026-05-24 amendment; see `dockerfiles/macos.README.md`. See ADR-0020 for that history and ADR-0030 for the current direction (Tart / Virtualization.framework).
