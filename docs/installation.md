...
Each binary ships with a `.sha256` sidecar for integrity verification. The sidecar lists the **published asset name** (e.g. `gsd-test-v1.8.0-darwin-arm64`), so after downloading the binary as `./gsd-test` (see your platform below) pair that hash with the local file:
