...
- **No automated reaping of leaked VMs.** Docker Benches have a Tier-2 reaper (`internal/reaper`) that sweeps stale containers by `--name`/label + deadline (ADR-0029). Tart VMs started by `tartpipeline.Pipeline` carry no equivalent reaper-visible identity, and there is no Tart-aware reaper sweep. A run that crashes before its deferred `Cleanup` fires (process killed, host rebooted) leaks a running Tart VM with no automated cleanup. See the [how-to guide](macos-tart-how-to.md#how-to-clean-up-a-leaked-vm-by-hand) to remove one manually.
