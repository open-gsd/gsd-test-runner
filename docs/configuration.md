...
| `runtime` | `string` | No | `"docker"` | Container runtime. `"docker"` is the default for all Benches today (Linux, Windows, and macOS). `"container"` is reserved for future Apple Containers support (requires macOS 26; not usable today — see ADR-0020). `"tart"` selects Tart (Cirrus Labs' Virtualization.framework-based CLI) for a genuine macOS-native guest on macOS Benches — see [ADR-0030](adr/0030-macos-bench-via-tart.md). As of this change, config/selection support exists but the actual Tart execution path is not yet implemented (tracked in issue #134). |
